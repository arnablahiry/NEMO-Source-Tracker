...
$$c_j = h^{(j)} \star c_{j-1}, \qquad h^{(j)}[k] = \frac{1}{16}\lbrack 1,\, 4,\, 6,\, 4,\, 1\rbrack \text{ at step } 2^{j-1}$$
...
$$\mathcal{A}\lbrack M, \mathbf{v}\rbrack(y', x') = \sum_{(y,x)\,:\,M(y,x)=1} \delta\!\left(y' - \lfloor y + v_r(y,x) \rceil\right) \delta\!\left(x' - \lfloor x + v_c(y,x) \rceil\right)$$
...
$$\mathcal{C}_{ij} = -\sum_{y,x} \mathcal{A}\lbrack M_i^{\rm adv}, \mathbf{v}\rbrack(y,x) \cdot M_j^{(c_{\rm tgt})}(y,x)$$
...
$$\text{IoU}_{\rm flow} = \frac{1}{|C|-1} \sum_{i=1}^{|C|-1} \frac{\bigl|\bigl(\mathcal{A}\lbrack M^{(c_i)}, \mathbf{v}_{i,i+1}\rbrack > 0.3\bigr) \cap M^{(c_{i+1})}\bigr|}{\bigl|\bigl(\mathcal{A}\lbrack M^{(c_i)}, \mathbf{v}_{i,i+1}\rbrack > 0.3\bigr) \cup M^{(c_{i+1})}\bigr|}$$
